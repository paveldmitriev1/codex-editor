...
def run_all(chapter_id: str) -> dict:
    ...
    results = {}
    seen = []
    # Pavel 2026-05-25: ВСЕ персоны отключены. «мы наверное не будем включать
    # агентов которые в роли людей как илон маск. это создает шум и непонятки».
    # Один Мастер + статические критики (voice_purity/ai_tells/mystical_depth/rhythm/sacred_lexicon)
    # + Соавтор + Voice Diff достаточно. Persons придумывают чужие точки зрения и
    # перемешивают ценные наблюдения с шумом.
    keys = []
    for k in keys:
